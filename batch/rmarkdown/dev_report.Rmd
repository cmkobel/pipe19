...
Herunder forsøges at fremstille aktuelle billede af disse mutations*konstellationer*.

```{r constellations}
MOIs <- c("S:H69-", "S:V70-", # Mutations of interest
          "S:E484K", "S:N501Y", "S:Y453F") 
mutations_wide <- ready %>%
  select(raw_full_name, lineage, aaSubstitutions, aaDeletions) %>%
  separate_rows(aaSubstitutions, sep = ",") %>% 
  mutate(sub_present = 1L, lin_present = 1L) %>% 
  pivot_wider(names_from = aaSubstitutions, values_from = sub_present, values_fill = 0L) %>% 
  separate_rows(aaDeletions, sep = ",") %>% 
  mutate(del_present = 1L) %>% 
  pivot_wider(names_from = aaDeletions, values_from = del_present, values_fill = 0L) %>% 
  select(-`NA`) %>% # Mange prøver har ingen deletioner, nogle har ikke substitutioner
  pivot_wider(names_from = lineage, values_from = lin_present, values_fill = 0L) %>% 
  as.data.frame() 
...
```{r}
svartid <- ready %>% 
  mutate(svartid = lubridate::ymd(batch) - modtaget + 1) 
...
qc <- illumina %>%
  ...
  mutate(qc.overallStatus = factor(qc.overallStatus,
                                   ...
                                   ordered = TRUE),
         fail_category = cut(totalMissing_interpreted, 
...
```{r qc-2}
qc %>% 
  ggplot(aes(lowest_ct, totalMissing_interpreted)) + 
...
```{r qc-5}
qc %>% 
  mutate_at(vars(lowest_ct, totalMissing_interpreted), as.integer) %>% 
  mutate(eurofins = if_else(`SARS2 Eurofins COV` == "Positiv", "Eurofins", "In-house", missing = "In-house")) %>% 
  ggplot(aes(totalMissing_interpreted, colour = eurofins)) + 
  stat_ecdf() + 
